...
class SemanticsSuite extends AnyFunSuite {
  ...
  val solverNames: Seq[String] = {
    val withoutCache = ((if (SolverFactory.hasNativeZ3) Seq("nativez3", "unrollz3") else Nil) ++
      (if (SolverFactory.hasZ3) Seq("smt-z3") else Nil) ++
      (if (SolverFactory.hasCVC4) Seq("smt-cvc4") else Nil) ++
      Seq("princess"))

    withoutCache ++ withoutCache.map("caching:" + _)
  ...
  protected def filterSolvers(
      ctx: Context,
      princess: Boolean = false,
      cvc4: Boolean = false,
  ...
  test("BitVector Casts", filterSolvers(_, princess = true)) { ctx =>
    ...
    check(
      ...
      Equals(
        ...
        Int64Literal(Int.MaxValue + b.toInt)
      ),
      BooleanLiteral(false)
    ) // mind the `toInt` instead of `toLong`
  }
  ...
  test("solve bitwise operations", filterSolvers(_, princess = true)) { ctx =>
    ...
    check(
      s,
      BVNarrowingCast(
        BVAnd(BVWideningCast(Int8Literal(1), Int32Type()), BVWideningCast(Int8Literal(2), Int32Type())),
        Int8Type()
      ),
      Int8Literal(0)
    )
    ...
    assert(
      s.solveVALID(
        let(
          "s" :: SetType(Int32Type()),
          SetUnion(FiniteSet(Seq(Int32Literal(1), Int32Literal(2)), Int32Type()), FiniteSet(Seq(Int32Literal(1)), Int32Type()))
        )(s => or(Equals(v, Int32Literal(1)), Equals(v, Int32Literal(2)), Not(ElementOfSet(v, s))))
      ) contains true
    )
