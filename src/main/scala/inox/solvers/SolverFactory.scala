...
object SolverFactory {
  ...
  def create[S1 <: Solver](
      p: Program
  )(nme: String, builder: () => S1 { val program: p.type }): SolverFactory { val program: p.type; type S = S1 { val program: p.type } } = {
  ...
  // remove "caching:" prefix
  private def removeCaching(name: String): String = name.drop(8)

  def getFromName(name: String, force: Boolean = false)(p: Program, ctx: Context)(enc: ProgramTransformer {
    ...
    val caching = name.startsWith("caching:")
    val noCachingPrefixName = if (caching) name.drop(8) else name

    val nonIncremental = noCachingPrefixName.startsWith("no-inc:")
    val noPrefixName = if (nonIncremental) removeNoInc(noCachingPrefixName) else noCachingPrefixName
    ...
    def nonIncrementalWrap[T, M](targetProgram: Program)(
      ...
      if (nonIncremental) {
        class NonIncrementalImpl(override val program: targetProgram.type, override val context: inox.Context) extends NonIncrementalSolver {
    ...
    val resultingSolverFactory: SolverFactory { val program: p.type; type S <: TimeoutSolver { val program: p.type } } = finalName match {
      case "nativez3" =>
        create(p)(
          finalName, {
            val chooseEnc = ChooseEncoder(p)(enc)
            class NativeZ3Impl(override val program: p.type)(override val enc: transformers.ProgramTransformer {
        ...
        create(p)(
          finalName, {
            ...
            class NativeZ3OptImpl(override val program: p.type)
                extends z3.Z3Unrolling(program, ctx, enc, chooseEnc)
                with z3.NativeZ3Optimizer
                with TimeoutSolver
      ...
      case "unrollz3" =>
        create(p)(
          finalName, {
            class UnrollZ3Impl(override val program: p.type)(override val enc: transformers.ProgramTransformer {
              val sourceProgram: program.type
              val targetProgram: Program { val trees: inox.trees.type }
            })(override val chooses: ChooseEncoder {
              val program: p.type
              ...
              class Underlying(override val program: targetProgram.type) extends z3.UninterpretedZ3Solver(targetProgram, ctx)
      ...
      case _ if finalName == "smt-z3" || finalName.startsWith("smt-z3:") =>
        create(p)(
          finalName, {
            class SMTZ3Impl(override val program: p.type)(override val enc: transformers.ProgramTransformer {
              val sourceProgram: program.type
              val targetProgram: Program { val trees: inox.trees.type }
            })(override val chooses: ChooseEncoder {
              val program: p.type
              ...
              class Underlying(override val program: targetProgram.type) extends smtlib.SMTLIBSolver(program, context) with smtlib.Z3Solver {
      ...
      case "smt-cvc4" =>
        create(p)(
          finalName, {
            ...
            class SMTCVC4Impl(override val program: p.type)(override val enc: transformers.ProgramTransformer {
              val sourceProgram: program.type
              val targetProgram: Program { val trees: inox.trees.type }
            })(override val chooses: ChooseEncoder {
              val program: p.type
              ...
              class Underlying(override val program: targetProgram.type) extends smtlib.SMTLIBSolver(program, ctx) with smtlib.CVC4Solver
      ...
      case "princess" =>
        create(p)(
          finalName, {
            val chooseEnc = ChooseEncoder(p)(enc)
            class PrincessImpl(override val program: p.type) extends princess.PrincessSolver(p, ctx, enc, chooseEnc) with TimeoutSolver
    ...
    if caching then CachingSolverFactory(p)(resultingSolverFactory)
    else resultingSolverFactory
  }
  ...
  def getFromSettings(p: Program, ctx: Context)(enc: ProgramTransformer {
    val sourceProgram: p.type
    val targetProgram: Program { val trees: inox.trees.type }
  })(using sem: p.Semantics): SolverFactory { val program: p.type; type S <: TimeoutSolver { val program: p.type } } = {
    ctx.options.findOption(optSelectedSolvers) match {
      case None =>
        optSelectedSolvers.default.toSeq match {
  ...
  def optimizer(p: InoxProgram, ctx: Context): SolverFactory {
    ...
    (solversOpt getOrElse optSelectedSolvers.default).toSeq match {
      ...
      case Seq(single) =>
        ...
        getFromName(name, force = solversOpt.isDefined)(p, ctx)(ProgramEncoder.empty(p))(using p.getSemantics).asInstanceOf[
          SolverFactory {
            val program: p.type
            type S <: Optimizer with TimeoutSolver { val program: p.type }
          }
